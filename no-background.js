...
// icon-color: yellow; icon-glyph: th-list;
// Variables used by Scriptable.
// These must be at the very top of the file. Do not edit.
...
exports.generateSlices = async function ({ caller = 'none' }) {
  ...
  for (var i = 0; i < families.length; i++) {
    ...
    for (var c = 0; c < crops.length; c++) {
      ...
      const imgName = `${device_id}-${appearance}-${widgetSize}-${crop.pos}.tiff`
...
exports.getSlice = async function (name) {
  ...
  let position = name
  //log(position)
  const imgPath = fm.joinPath(cachePath, `${device_id}-${appearance}-${position}.tiff`)
  if (!fm.fileExists(imgPath)) {
...
exports.getPathForSlice = async function (slice_name) {
  ...
  let imgPath = fm.joinPath(cachePath,
    `${device_id}-${appearance}-${slice_name}.tiff`)
...
exports.getSliceForWidget = async function (
  ...
  if (!position) {
    ...
    const testImage = fm.joinPath(cachePath, `${device_id}-${appearance}-medium-top.tiff`)
  ...
  const imgPath = fm.joinPath(cachePath, `${device_id}-${appearance}-${position}.tiff`)
  if (!fm.fileExists(imgPath)) {
    log(`Slice does not exists - ${device_id}-${appearance}-${position}.tiff`)
    return null
  }
